...
pub fn get_explanation_for_api_combo(apis: &[&str]) -> Vec<PlainEnglishFinding> {
    ...
    let has_alloc = lower.iter().any(|a| a == "virtualallocex" || a == "ntallocatevirtualmemory");
    let has_write = lower.iter().any(|a| a == "writeprocessmemory");
    let has_thread = lower
        .iter()
        .any(|a| a == "createremotethread" || a == "ntcreatethreadex" || a == "rtlcreateuserthread");
    if has_alloc && has_write && has_thread {
        if let Some(f) = get_explanation("process_injection_combo") {
            findings.push(f);
        }
    }
    ...
    if has_dump && has_open {
        if let Some(f) = get_explanation("lsass_dump_combo") {
            findings.push(f);
        }
    }
    ...
    let has_hook = lower
        .iter()
        .any(|a| a.starts_with("setwindowshookex"));
    if has_key || has_hook {
        if let Some(f) = get_explanation("keylogger_combo") {
            findings.push(f);
        }
    }
    ...
    if has_debug {
        if let Some(f) = get_explanation("debugger_check") {
            findings.push(f);
        }
    }
    ...
    if has_reg || has_svc {
        if let Some(f) = get_explanation("persistence_combo") {
            findings.push(f);
        }
    }

    // Privilege escalation
    if lower.iter().any(|a| a == "adjusttokenprivileges" || a == "impersonateloggedonuser") {
        if let Some(f) = get_explanation("privilege_escalation") {
            findings.push(f);
        }
    }
    ...
    if has_net {
        if let Some(f) = get_explanation("network_combo") {
            findings.push(f);
        }
    }

    // Clipboard
    if lower.iter().any(|a| a == "getclipboarddata" || a == "openclipboard") {
        if let Some(f) = get_explanation("clipboard_access") {
            findings.push(f);
        }
    }
...
mod tests {
    ...
    fn test_all_findings_have_nonempty_content() {
        for (key, finding) in get_map() {
            ...
            assert!(!finding.explanation.is_empty(), "Empty explanation for key {key}");
        }
    }
    ...
    fn test_injection_combo_returns_critical() {
        ...
        assert!(findings.iter().any(|f| f.confidence == ConfidenceLevel::Critical));
        assert!(findings.iter().any(|f| {
            f.mitre_technique_id.as_deref() == Some("T1055.001")
        }));
    }
    ...
    fn test_get_explanation_count() {
        assert!(get_map().len() >= 20, "Should have at least 20 explanations");
    }
}
