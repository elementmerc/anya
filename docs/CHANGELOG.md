...
- Releases now publish a consolidated `SHA256SUMS.txt` file covering every binary asset; verify with `sha256sum -c SHA256SUMS.txt`. Individual `.sha256` sidecars also still ship for backward compatibility.
